...
from .file_utils import MyOpen, search_paths

def Bitter_simfile(MyEnv, confdata: dict, cad: Bitter):
    """
    generate sim files for Bitter
    """

    print(f'Bitter_simfile: {cad.name}')
...
def Bitter_setup(MyEnv, confdata: dict, cad: Bitter, method_data: List, templates: dict, debug: bool = False):
    ...
    if method_data[2] == "Axi":
        ...
        meanT_data.append({"header": f"MeanT_{name}", "markers": {"name": f"{name}_B%1%", "index1": index_Bitters}})
    else:
        print("bitter3D post not implemented")

    if debug:
        print(f'meanT_data: {meanT_data}')
